...
class JsonMigrationTarget implements MigrationTarget {
  ...
  Future<void> saveInvoice(InvoiceModel invoice) async {
    ...
    // Save to file
    await _saveToFile(
        'invoices.json', _invoices.map((i) => i.toMap()).toList());
  }
  ...
  Future<void> saveCustomer(Customer customer) async {
    ...
    // Save to file
    await _saveToFile(
        'customers.json', _customers.map((c) => c.toMap()).toList());
  }
  ...
  Future<void> saveGSTR1(GSTR1 gstr1) async {
    ...
    // Save to file
    await _saveToFile(
        'gstr1.json', _gstr1Returns.map((g) => g.toJson()).toList());
  }
  ...
  Future<void> saveGSTR3B(GSTR3B gstr3b) async {
    ...
    // Save to file
    await _saveToFile(
        'gstr3b.json', _gstr3bReturns.map((g) => g.toJson()).toList());
  }
  ...
  Future<void> saveGSTR9(GSTR9 gstr9) async {
    ...
    // Save to file
    await _saveToFile(
        'gstr9.json', _gstr9Returns.map((g) => g.toJson()).toList());
  }
  ...
  Future<void> saveGSTR9C(GSTR9C gstr9c) async {
    ...
    // Save to file
    await _saveToFile(
        'gstr9c.json', _gstr9cReturns.map((g) => g.toJson()).toList());
  }
  ...
  Future<void> saveSetting(String key, String value) async {
    ...
    // Save to file
    final settingsList = _settings.entries
        .map((e) => {
              'key': e.key,
              'value': e.value,
