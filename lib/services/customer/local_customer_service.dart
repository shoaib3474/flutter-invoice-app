...
import 'package:flutter_invoice_app/models/customer/customer_extensions.dart';
...
class LocalCustomerService {
  ...
  static List<Customer> getAllCustomers(
      {bool includeB2B = true, bool includeB2C = true}) {
  ...
  static Map<String, String> validateCustomer(Customer customer) {
    ...
    } else if (!RegExp(r'^[6-9]\d{9}$').hasMatch(customer.mobile.toString())) {
  ...
  static void updateCustomer(Customer customer) {
    final existingData =
        LocalStorageService.getCustomerByMobile(customer.mobile.toString());
    final customerData = customer.toMap();
  ...
  static Map<String, dynamic> getCustomerStats() {
    ...
    final b2bCustomers =
        customers.where((c) => c.gstin?.isNotEmpty == true).length;
    ...
    return {
      ...
      'withAddress':
          customers.where((c) => c.address?.isNotEmpty == true).length,
    };
  }
