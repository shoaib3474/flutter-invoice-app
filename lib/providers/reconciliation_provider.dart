...
class ReconciliationProvider with ChangeNotifier {
  ...
  // Getters
  List<ReconciliationResult> get reconciliationResults => _reconciliationResults;
  ...
  Future<void> loadReconciliationResultsByType(ReconciliationType type) async {
    ...
    try {
      _reconciliationResults = await _repository.getReconciliationResultsByType(type);
      _error = null;
    } catch (e) {
      _error = 'Failed to load reconciliation results: ${e.toString()}';
    } finally {
  ...
  Future<void> reconcileGSTR1WithGSTR2A(String gstin, String period) async {
    ...
    try {
      _selectedResult = await _repository.reconcileGSTR1WithGSTR2A(gstin, period);
  ...
  Future<void> reconcileGSTR1WithGSTR3B(String gstin, String period) async {
    ...
    try {
      _selectedResult = await _repository.reconcileGSTR1WithGSTR3B(gstin, period);
      await loadAllReconciliationResults();
      _error = null;
    } catch (e) {
  ...
  Future<void> reconcileGSTR2AWithGSTR2B(String gstin, String period) async {
    ...
    try {
      _selectedResult = await _repository.reconcileGSTR2AWithGSTR2B(gstin, period);
      await loadAllReconciliationResults();
      _error = null;
    } catch (e) {
  ...
  Future<void> reconcileGSTR2BWithGSTR3B(String gstin, String period) async {
    ...
    try {
      _selectedResult = await _repository.reconcileGSTR2BWithGSTR3B(gstin, period);
      await loadAllReconciliationResults();
      _error = null;
    } catch (e) {
  ...
  Future<void> performComprehensiveReconciliation(String gstin, String period) async {
    _setLoading(true);
    
    try {
      _selectedResult = await _repository.performComprehensiveReconciliation(gstin, period);
      await loadAllReconciliationResults();
      _error = null;
    } catch (e) {
      _error = 'Failed to perform comprehensive reconciliation: ${e.toString()}';
    } finally {
      _setLoading(false);
    }
  }
