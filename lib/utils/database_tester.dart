...
class DatabaseTester {
  final SupabaseClient _client = SupabaseConfig.client as SupabaseClient;
  ...
  Future<bool> testConnection() async {
    try {
      ...
      final response =
          await _client.from('health_check').select().limit(1).execute();
  ...
  Future<Map<String, bool>> testTables() async {
    ...
    for (final table in tables) {
      try {
        final response =
            await _client.from(table).select('count').limit(1).execute();
  ...
  Future<bool> testWrite() async {
    try {
      ...
      // Insert test record
      final response = await _client.from('test_writes').insert({
        'id': testId,
      ...
      await _client.from('test_writes').delete().eq('id', testId).execute();
